...
namespace ns_http //new
{
	std::string get(sol::this_state s, std::string& host, std::string& link)
	{
		...
		{
			Lua_manager->unload_script(get_current_script_id(s));
			logs->add(crypt_str("HTTP REQUESTS DONT WORK!!!!!"), Color::Red, crypt_str("[ LUA ] "), true);
...
{
	try {
		// Ensure Lua_manager is properly initialized with vectors
		if (!Lua_manager) {
			// Global instance wasn't created yet, create a temporary one for this call
			logs->add(crypt_str("Lua error: Lua_manager not initialized, creating temporary instance"), 
				Color::Yellow, crypt_str("[ LUA ] "), true);
			
			// Create a new c_lua object and point Lua_manager to it
			c_lua* new_manager = new c_lua();
			Lua_manager.set(new_manager);
		}
		
		// Double-check that all vectors are properly initialized
		if (Lua_manager.get() == this) {
			// This is a safety check to ensure vectors are created
			elements.clear();
			loaded.clear();
			scripts.clear();
			dme_buf.clear();
			...
			logs->add(crypt_str("Lua error: Lua_manager not properly initialized"), 
				Color::Red, crypt_str("[ LUA ] "), true);
		}
	}
	catch (const std::exception& e) {
		logs->add(crypt_str("Lua error: Exception in initialize: ") + std::string(e.what()), 
			Color::Red, crypt_str("[ LUA ] "), true);
	}
	catch (...) {
		logs->add(crypt_str("Lua error: Failed to initialize Lua manager"), 
			Color::Red, crypt_str("[ LUA ] "), true);
	}
}
...
int c_lua::get_script_id(const std::string& name)
{
	try {
		...
		logs->add(crypt_str("Lua error: Failed to get script ID"), Color::Red, crypt_str("[ LUA ] "), true);
	}
...
int c_lua::get_script_id_by_path(const std::string& path)
{
	try {
		...
		logs->add(crypt_str("Lua error: Failed to get script ID by path"), Color::Red, crypt_str("[ LUA ] "), true);
	}
...
{
	// Add safety guard to check if 'this' is valid
	try {
		// Create new vectors to work with to avoid modifying existing ones in case of error
		std::vector<bool> new_loaded;
		std::vector<std::string> new_scripts;
		std::vector<std::filesystem::path> new_pathes;
		std::vector<std::vector<std::pair<std::string, menu_item>>> new_items;
		
		// Copy old data if possible
		std::vector<bool> oldLoaded;
		std::vector<std::string> oldScripts;
		
		bool has_scripts = false;
		try {
			has_scripts = scripts.size() > 0;
		}
		catch (...) {
			logs->add(crypt_str("Lua error: scripts vector is in invalid state"), Color::Red, crypt_str("[ LUA ] "), true);
			has_scripts = false;
		}
		...
		if (has_scripts) {
			try {
				...
				logs->add(crypt_str("Lua error: Failed to copy script vectors"), Color::Red, crypt_str("[ LUA ] "), true);
			}
		}
			...
			try {
				for (auto& entry : std::filesystem::directory_iterator(folder))
				{
					if (entry.path().extension() == crypt_str(".lua") || entry.path().extension() == crypt_str(".luac"))
					{
						...
						// Match against old scripts if they exist
						if (!oldScripts.empty()) {
							for (size_t i = 0; i < oldScripts.size(); i++) {
								if (filename == oldScripts[i]) {
									script_loaded = oldLoaded[i];
									found_match = true;
									break;
								}
							}
						...
						// Add to our new vectors
						new_loaded.push_back(script_loaded);
						new_pathes.push_back(entry_path);
						new_scripts.push_back(filename);
						new_items.push_back(std::vector<std::pair<std::string, menu_item>>());
					}
				}
				
				// Only after successful processing, update the class vectors
				loaded = new_loaded;
				scripts = new_scripts;
				pathes = new_pathes;
				items = new_items;
				
				// Clear other vectors that depend on script data
				try {
					ns_console::convars.clear();
				}
				catch (...) {
					logs->add(crypt_str("Lua error: Failed to clear convars"), Color::Red, crypt_str("[ LUA ] "), true);
				}
			}
			catch (const std::exception& e) {
				logs->add(crypt_str("Lua error: Exception processing scripts folder: ") + std::string(e.what()), 
					Color::Red, crypt_str("[ LUA ] "), true);
			}
			catch (...) {
				logs->add(crypt_str("Lua error: Unknown exception processing scripts folder"), 
					Color::Red, crypt_str("[ LUA ] "), true);
			}
		}
		else {
			logs->add(crypt_str("Lua error: Could not get AppData folder path"), Color::Red, crypt_str("[ LUA ] "), true);
		}
	}
	catch (const std::exception& e) {
		logs->add(crypt_str("Lua error: Exception in refresh_scripts: ") + std::string(e.what()), 
			Color::Red, crypt_str("[ LUA ] "), true);
	}
	catch (...) {
		logs->add(crypt_str("Lua error: Failed to refresh scripts"), Color::Red, crypt_str("[ LUA ] "), true);
	}
}
...
void c_lua::load_script(int id)
{
	...
	// Add bounds checking for loaded vector
	if (id < 0 || id >= static_cast<int>(loaded.size())) {
		logs->add(crypt_str("Lua error: Invalid script ID"), Color::Red, crypt_str("[ LUA ] "), true);
		return;
	}
...
void c_lua::unload_script(int id)
{
	...
	// Add bounds checking for loaded vector
	if (id < 0 || id >= static_cast<int>(loaded.size())) {
		logs->add(crypt_str("Lua error: Invalid script ID for unloading"), Color::Red, crypt_str("[ LUA ] "), true);
		return;
	}
	...
	// Fix potentially unsafe vector iteration - use reverse iteration to safely erase
	for (int i = static_cast<int>(Lua_manager->fonts.size()) - 1; i >= 0; i--)
	{
	...
	// Add extra safety check for items vector
	if (id < items.size()) {
		items.at(id).clear();
		loaded.at(id) = false;
	}
...
std::string c_lua::get_script_path(int id)
{
	...
	// Add bounds checking for pathes vector
	if (id < 0 || id >= static_cast<int>(pathes.size())) {
		logs->add(crypt_str("Lua error: Invalid script path ID"), Color::Red, crypt_str("[ LUA ] "), true);
		return crypt_str("");
	}
