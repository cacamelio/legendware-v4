...
DWORD WINAPI main_function(LPVOID module_address)
{
	...
	if (!std::filesystem::exists(path_resources + crypt_str("Configs\\")))
		std::filesystem::create_directories(path_resources + crypt_str("Configs\\"));

	if (!std::filesystem::exists(path_resources + crypt_str("Scripts\\")))
		std::filesystem::create_directories(path_resources + crypt_str("Scripts\\"));
